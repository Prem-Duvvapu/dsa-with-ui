 ...
 * second path exists.
 *
 * <p>Paired with {@code cycle-undirected-bfs}: this tracer's default is a tree (no cycle)
 * and its alternate has one, the opposite pairing from that BFS tracer, so across the two
...
public class CycleUndirectedDfsTracer implements AlgorithmTracer {
    ...
    /** Closing the path back on itself with a 0-3 edge turns the tree into a cycle. */
    @Override

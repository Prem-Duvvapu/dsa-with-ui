 ...
 *
 * <p>Paired with {@code cycle-undirected-dfs}: this tracer's default HAS a cycle and its
 * alternate does not, the opposite pairing from that one, so across the two tracers both
 * branches are exercised by a genuinely different graph shape rather than a relabelled copy
 * of the same one.
 */
@Component
public class CycleUndirectedBfsTracer implements AlgorithmTracer {

    @Override
    public String id() {
        return "cycle-undirected-bfs";
    }
    ...
    /** A star - every edge shares vertex 0, so there is exactly one path between any pair. */
    @Override

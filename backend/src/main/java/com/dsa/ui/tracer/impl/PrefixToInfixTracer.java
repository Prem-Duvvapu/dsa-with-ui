...
/**
 * Prefix to Infix conversion. Scan right to left: operands push as strings,
 * operators pop two operands and push "(op1 operator op2)".
 */
@Component
public class PrefixToInfixTracer implements AlgorithmTracer {
    ...
    public void run(Inputs in, StepEmitter emit) {
        ...
        emit.at("init")
                .say("Convert prefix \"%s\" to infix. Scan right to left.", s)
                .var("expression", s)
        ...
        for (int i = s.length() - 1; i >= 0; i--) {
            ...
            if (Character.isLetter(c)) {
                stack.push(String.valueOf(c));
                emit.at("pushOperand")
                        .say("s[%d] = '%c' is an operand — push \"%c\" onto the stack.", i, c, c)

    ...
    public InputSpec inputSpec() {
        return InputSpec.of(
                InputField.of("s", FieldType.STRING)
                        ...
                        // on the input every visitor sees first. This one shrinks three
                        // times on the way to the same answer of 4.
    ...
    public void run(Inputs in, StepEmitter emit) {
        ...
        for (int right = 0; right < s.length(); right++) {
            ...
            if (windowLen - maxFreq > k) {
                ...
                left++;
                emit.at("shrink")
                        .say("Window of %d needs more than %d replacement%s - shrink past '%c'. "
                                        + "Window now starts at %d.",
                                windowLen, k, k == 1 ? "" : "s", dropped, left)
                        .var("left", left)
                        .arrayState(windowState(s, left, right)).step();
            }
            ...
            emit.at("windowComplete")
                    .say("Window [%d,%d] is valid with at most %d replacement%s. Best so far: %d.",
                            left, right, k, k == 1 ? "" : "s", maxLen)
                    .var("windowLen", right - left + 1).var("maxLen", maxLen)
                    .arrayState(windowState(s, left, right)).step();
        }

        emit.at("done")
                .say("Every character processed. Longest window achievable with %d replacement%s: %d.",
                        k, k == 1 ? "" : "s", maxLen)
                .var("answer", maxLen)
